...
(function($) {
	...
	$.fn.carouFredSel = function(options, configs) {
		...
		$cfs._cfs_bind_events = function() {
			...
			$cfs.bind(cf_e('prev', conf)+' '+cf_e('next', conf), function(e, obj, num, clb) {
				...
				if (typeof num != 'number') {
					...
					if (num == 'visible') {
						if (!opts.items.visibleConf.variable) num = opts.items.visible;
					}
				}
			...
			$cfs.bind(cf_e('_cfs_slide_prev', conf, false), function(e, sO, nI) {
				...
				if (opts.items.visibleConf.variable) {
					...
					opts.items.visibleConf.old = opts.items.visible;
					opts.items.visible = cf_getItemsAdjust(vI, opts, opts.items.visibleConf.adjust);
				}
			...
			$cfs.bind(cf_e('updatePageStatus', conf), function(e, build, sizes) {
				...
				opts.pagination.container.each(function() {
					$(this).children().removeClass('selected').eq($cfs.triggerHandler(cf_e('currentPage', conf))).addClass('selected');
				});
				return true;
			});
		...
		if (opts.items.start != 0) {
			...
			$cfs.trigger(cf_e('slideTo', conf), [s, 0, true, { fx: 'none' }]);
		}
